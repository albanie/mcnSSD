...
% Download data
if ~exist(fullfile(opts.dataDir,'Annotations')) 
    download(opts) ; 
end
...
if opts.includeTest 
    [imdb, index] = addImageSet(opts, imdb, index, 'test', 3) ; 
end
...
if opts.includeSegmentation
  ...
  if opts.includeTest 
      [imdb, index] = addSegmentationSet(opts, imdb, index, 'test', 3) ; 
  end
end
...
switch opts.edition
  ...
  case '12'
    url=sprintf('%s/pascal/VOC/voc2012/VOCtrainval_11-May-2012.tar', baseUrl) ;
end
...
if ~exist(opts.archiveDir), 
    mkdir(opts.archiveDir) ; 
end
  ...
  if ~exist(archivePath)
    try 
      ...
      urlwrite(testUrl, archivePath) ;
       ...
       error(strcat('Cannot download the test data automatically. Please ', ...
           ' download the file %s manually from the PASCAL test server.'), ...
             archivePath) ;
     end
  end
  fprintf('%s: decompressing and rearranging %s\n', mfilename, archivePath) ;
...
for j=1:numel(imdb.images.id)
  ...
  if ~exist(annoPath, 'file')
    if imdb.images.classification(j) && imdb.images.set(j) ~= 3
      warning(strcat('Could not find detection annotations for ', ...
                       'image ''%s''. Skipping.'), name) ;
    end
    continue ;
  end
